...
from utils.load_data_rgb_abs_action_fast_gripper_finetuned_attn_dalle_2_obj_fov40 import DMPDatasetEERandTarXYLang, pad_collate_xy_lang
...
def train(writer, name, epoch_idx, data_loader, model, 
    ...
    for idx, (img, target, target_2, joint_angles, ee_pos, ee_traj, ee_xy, length, target_pos, target_2_pos, phis, mask, target_1_xy, target_2_xy, sentence, joint_angles_traj, displacement, displacement_2) in enumerate(data_loader):
        ...
        # Log and print
        if (epoch_idx * len(data_loader) + idx) % 100 == 0:
            writer.add_scalar('train loss', loss.item(), global_step=epoch_idx * len(data_loader) + idx)
        print(f'epoch {epoch_idx}, step {idx} / {len(data_loader)}, l_all {loss.item():.2f}')
...
def test(writer, name, epoch_idx, data_loader, model, criterion, train_dataset_size, stage, print_attention_map=False, train_split=False):
    with torch.no_grad():
        ...
        for idx, (img, target, target_2, joint_angles, ee_pos, ee_traj, ee_xy, length, target_pos, target_2_pos, phis, mask, target_1_xy, target_2_xy, sentence, joint_angles_traj, displacement, displacement_2) in enumerate(data_loader):
            ...
            idx += 1
            print(f'{idx} / {len(data_loader)}', f'err traj {(error_trajectory / num_trajpoints).item():.4f}')
...
def main(writer, name, batch_size=96):
    ...
    val_dirs = [os.path.join(val_set_path, x) for x in os.listdir(os.path.join(data_source_root, val_set_path))]

    print(train_dirs)

    dataset_train_dmp = DMPDatasetEERandTarXYLang(train_dirs, random=False, length_total=120,
    ...
    dataset_test_dmp = DMPDatasetEERandTarXYLang(val_dirs, random=False, length_total=120, 
        source_root = data_source_root,
        target_root = data_target_root)
...
if __name__ == '__main__':
    name = 'train-baseline-bcz-film-resnet-huberloss-dalle-2-obj-fov40'
