...
class DMPDatasetEERandTarXYLang(Dataset):
    ...
    target_root = '/media/yzhou298/e/'):
        ...
        self.action_inst_to_verb = {
            'left_to': ['move', 'put', 'get', 'place'],
            'right_to': ['move', 'put', 'get', 'place']
        }

        self.action_inst_to_adv = {
            'left_to': ['to the left of', 'left to'],
            'right_to': ['to the right of', 'right to']
        }
    ...
    def adv_phrase_template(self, action_inst):
        if action_inst is None:
            action_inst = random.choice(list(self.action_inst_to_verb.keys()))
        adv = random.choice(self.action_inst_to_adv[action_inst])
        return adv.strip()

    def sentence_template(self, target_1, target_2, action_inst=None):
        sentence = ''
        sentence = sentence + self.verb_phrase_template(action_inst)
        sentence = sentence + ' ' + self.noun_phrase_template(target_1)
        sentence = sentence + ' ' + self.adv_phrase_template(action_inst)
        sentence = sentence + ' ' + self.noun_phrase_template(target_2)
        return sentence.strip()

    def xyz_to_xy(self, xyz, cam_pos=np.array([0, 1.5, 1]), theta=np.array([-1, 0, 3.14]), fov=40, output_size_x=224, output_size_y=224):
        """
    ...
    def __getitem__(self, index):
        ...
        if self.random:
            target_1, target_2 = random.sample(list(self.trials[trial_idx]['displacement'].keys()), 2)
            action = None
        else:
            target_1 = self.trials[trial_idx]['goal_object'][0]
            target_2 = self.trials[trial_idx]['goal_object'][1]
        ...
        return img, target_1, target_2, joint_angles, ee_pos, ee_traj, ee_xy, length, target_1_pos, target_2_pos, phis, mask, target_1_xy, target_2_xy, sentence[0], joint_angles_traj, displacement_1, displacement_2
